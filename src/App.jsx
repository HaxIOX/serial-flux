...
export default function App() {
  ...
  const t = {
    // Background
    pageBg: isDark ? 'bg-[#010101]' : 'bg-[#FBFAF6]',
    windowBg: isDark ? 'bg-zinc-800/90' : 'bg-[#FFFEFB]/90',
    panelBg: isDark ? 'bg-zinc-800/60' : 'bg-[#FFFEFB]/85',
    sideBg: isDark ? 'bg-zinc-800/95' : 'bg-[#FFFEFB]/85',
    inputBg: isDark ? 'bg-zinc-700/40' : 'bg-[#FFFEFB]',
    logBg: isDark ? 'bg-black/5' : 'bg-[#FFFEFB]/70',
    // Text
    ...
    borderHover: isDark ? 'border-white/20' : 'border-indigo-300',
    // Accent (indigo for light, emerald for dark) - aligned with houtai.html
    accent: isDark ? 'emerald' : 'indigo',
    accentText: isDark ? 'text-emerald-400' : 'text-indigo-600',
    accentBg: isDark ? 'bg-emerald-500' : 'bg-indigo-600',
    accentBgLight: isDark ? 'bg-emerald-500/20' : 'bg-indigo-50',
    // TX/RX
    ...
    // Button
    btnPrimary: isDark ? 'from-emerald-600 to-emerald-500' : 'from-indigo-600 to-indigo-500',
    btnPrimaryHover: isDark ? 'hover:from-emerald-500 hover:to-emerald-400' : 'hover:from-indigo-500 hover:to-indigo-400',
  };

  return (
    <div className={`flex h-screen w-full items-center justify-center ${t.pageBg} ${t.text} font-sans ${isDark ? 'selection:bg-emerald-500/25' : 'selection:bg-indigo-100 selection:text-indigo-900'} overflow-hidden relative transition-colors duration-300`}>
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
         {isDark ? (
           ...
           <>
             <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-indigo-200/35 blur-[120px] rounded-full"></div>
             <div className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] bg-emerald-300/40 blur-[100px] rounded-full"></div>
             <div className="absolute top-[30%] right-[20%] w-[30%] h-[30%] bg-indigo-100/40 blur-[90px] rounded-full"></div>
           </>
         )}
      </div>
      ...
      <div className={`relative w-[75vw] h-[72vh] max-w-[960px] max-h-[640px] flex flex-col animate-in fade-in zoom-in-95 duration-700
        ${isDark ? 'shadow-[0_0_80px_-20px_rgba(16,185,129,0.2),0_40px_100px_-20px_rgba(0,0,0,1)]' : 'shadow-[0_0_40px_-20px_rgba(99,102,241,0.18),0_25px_80px_-20px_rgba(15,23,42,0.12)]'}
        rounded-2xl ${t.border} border ${t.windowBg} ${isDark ? 'backdrop-blur-3xl' : 'backdrop-blur-md'} overflow-hidden ring-1 ${isDark ? 'ring-white/10' : 'ring-indigo-100'} transition-colors duration-300`}>

        <div className={`absolute inset-0 pointer-events-none border ${isDark ? 'border-emerald-500/10' : 'border-indigo-200'} rounded-2xl opacity-40`}></div>
        ...
        <div className={`h-8 ${isDark ? 'bg-black/40' : 'bg-white/60'} border-b ${t.border} flex items-center justify-between px-4 select-none flex-none z-50`}>
            <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full ${isDark ? 'hover:bg-white/10' : 'hover:bg-slate-100'} transition-colors`}
                  title={isDark ? 'Light mode' : 'Dark mode'}
                >
                  <Sun size={12} className={isDark ? 'text-amber-400' : 'text-indigo-600'} />
                </button>
            </div>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <main className={`flex-1 flex flex-col min-w-0 relative ${t.logBg}`}>
            ...
            <div className={`absolute top-2.5 left-4 z-20 flex items-center gap-3 px-3 py-1.5 pr-5 rounded-full border ${t.border} backdrop-blur-md ${isDark ? 'bg-zinc-800/80' : 'bg-white/85'} shadow-xl ring-1 ${isDark ? 'ring-white/5' : 'ring-indigo-100/60'} scale-90 origin-left`}>
            ...
            <div className="absolute top-2.5 right-4 z-20 flex items-center gap-2 scale-90 origin-right">
                <div className="relative group">
                    ...
                    <input type="text" value={logFilter} onChange={e => setLogFilter(e.target.value)} placeholder="Filter..." className={`h-8 pl-9 pr-3 rounded-full border ${t.border} ${isDark ? 'bg-black/60' : 'bg-white'} text-[10px] w-28 focus:w-48 transition-all outline-none ${isDark ? 'focus:border-emerald-500/40' : 'focus:border-indigo-500'} shadow-inner placeholder:opacity-50`} />
                </div>

                <div className="flex items-center gap-1.5">
                    <div className={`flex ${isDark ? 'bg-black/70' : 'bg-white/80'} border ${t.border} rounded-full p-0.5 shadow-lg`}>
                        <button onClick={() => setViewMode('ascii')} className={`w-12 py-1 rounded-full text-[10px] font-black tracking-wider transition-all ${viewMode === 'ascii' ? (isDark ? 'bg-zinc-700 text-white' : `${t.accentBgLight} ${t.accentText}`) : t.textDim + ' hover:opacity-80'}`}>ASC</button>
                        <button onClick={() => setViewMode('hex')} className={`w-12 py-1 rounded-full text-[10px] font-black tracking-wider transition-all ${viewMode === 'hex' ? (isDark ? 'bg-zinc-700 text-white' : `${t.accentBgLight} ${t.accentText}`) : t.textDim + ' hover:opacity-80'}`}>HEX</button>
                    </div>
                    <div className={`flex ${isDark ? 'bg-black/70' : 'bg-white/80'} border ${t.border} rounded-full p-0.5 shadow-lg`}>
                ...
                <button
                  ...
                  className={`size-8 rounded-full flex items-center justify-center ${isDark ? 'bg-black/60 border-white/10' : 'bg-white border-slate-200'} border ${t.textMuted} ${isDark ? 'hover:text-emerald-400' : 'hover:text-indigo-600'} transition-all shadow-lg disabled:opacity-30 disabled:cursor-not-allowed`}
                  title="Download Log"
                >
                  <Download size={14} />
                </button>
              ...
              {logs.length === 0 ? (
                ...
                <div className="space-y-0.5">
                  {logs.filter(l => !logFilter || String(l.text).toLowerCase().includes(logFilter.toLowerCase())).map((log) => (
                    <div key={log.id} onClick={() => { navigator.clipboard.writeText(String(log.text)); setCopyFeedback("COPIED!"); setTimeout(()=>setCopyFeedback(null), 1000); }} className={`flex items-start gap-2 ${isDark ? 'hover:bg-white/[0.04]' : 'hover:bg-slate-50/70'} -mx-4 px-4 py-0.5 rounded cursor-pointer group transition-colors`}>
          ...
          <aside className={`w-[240px] ${t.sideBg} border-l ${t.border} flex flex-col z-20 relative p-3.5 space-y-3.5 overflow-y-auto custom-scrollbar transition-colors duration-300`}>
                ...
                <div className="space-y-2.5">
                   <div className="relative w-full" ref={baudRef}>
                      ...
                      {isBaudDropdownOpen && !isConnected && (
                          <div className={`absolute top-full left-0 right-0 mt-1 ${isDark ? 'bg-zinc-800 border-white/20 ring-white/10' : 'bg-white border-slate-200 ring-indigo-100/60'} border rounded-lg shadow-[0_10px_30px_rgba(0,0,0,0.3)] z-50 max-h-40 overflow-y-auto ring-1 custom-scrollbar`}>
                              {COMMON_BAUD_RATES.map(rate => (
                                  <button key={rate} onClick={() => { setBaudRate(rate); setIsBaudDropdownOpen(false); }} className={`w-full text-left px-3 py-2 text-[11px] font-mono font-bold ${isDark ? 'hover:bg-emerald-500/20 border-white/5' : 'hover:bg-slate-50 border-slate-100 text-slate-700'} transition-colors border-b last:border-0`}>{rate}</button>
                              ))}
                          </div>
                      )}
                   </div>
                   <button
                    ...
                    className={`w-full py-2.5 rounded-lg font-black text-[10px] tracking-[0.2em] transition-all shadow-xl active:scale-[0.98] ${isConnected ? 'bg-rose-500/10 text-rose-500 border border-rose-500/30 hover:bg-rose-500/20' : (isDark ? 'bg-emerald-600 hover:bg-emerald-500 shadow-emerald-500/10' : `bg-gradient-to-r ${t.btnPrimary} ${t.btnPrimaryHover} shadow-indigo-500/10`) + ' text-white'}`}
                   >
                 ...
                 {isEditingCmds ? (
                   <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1 custom-scrollbar">
                      {quickCommands.map(cmd => (
                        <div key={cmd.id} className={`flex items-center gap-1.5 p-2 ${isDark ? 'bg-zinc-800/60 border-white/10 border-l-emerald-500/50' : 'bg-white border-slate-200 border-l-indigo-400'} rounded-lg border border-l-2`}>
                           <input type="text" placeholder="Lbl" value={cmd.label} onChange={e => {const n=[...quickCommands]; n.find(c=>c.id===cmd.id).label=e.target.value; setQuickCommands(n);}} className={`w-12 shrink-0 ${isDark ? 'bg-zinc-800 border-white/10 text-zinc-200 focus:border-emerald-500/40' : 'bg-slate-50 border-slate-200 text-slate-700 focus:border-indigo-500'} border rounded px-1.5 py-1 text-[10px] outline-none font-bold`} />
                           <input type="text" placeholder="Cmd" value={cmd.cmd} onChange={e => {const n=[...quickCommands]; n.find(c=>c.id===cmd.id).cmd=e.target.value; setQuickCommands(n);}} className={`flex-1 min-w-0 ${isDark ? 'bg-zinc-800 border-white/10 text-emerald-400 focus:border-emerald-500/40' : 'bg-slate-50 border-slate-200 text-indigo-600 focus:border-indigo-500'} border rounded px-1.5 py-1 text-[10px] font-mono font-bold outline-none`} />
                      ...
                      {quickCommands.map(cmd => (
                        ...
                        >
                           <div className="overflow-hidden min-w-0">
                               ...
                               <div className={`text-[10px] font-mono font-medium ${isDark ? 'text-emerald-400/80' : 'text-indigo-600'} truncate mt-0.5`}>{cmd.cmd}</div>
                           </div>
                           <ChevronRight size={12} className={`${isDark ? 'text-zinc-600 group-hover:text-emerald-400' : 'text-slate-400 group-hover:text-indigo-500'} transition-colors shrink-0`} />
                        </button>
                      ))}
                   </div>
                 )}
            ...
            <div className={`mt-auto space-y-2 border-t ${t.border} pt-3`}>
                <div className="flex items-center justify-between px-1">
                    <div className="flex gap-2">
                        <button onClick={() => setAppendCRC(!appendCRC)} className={`px-2 py-0.5 rounded border text-[8px] font-black transition-all ${appendCRC ? (isDark ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40 shadow-[0_0_10px_rgba(16,185,129,0.1)]' : 'bg-indigo-50 text-indigo-600 border-indigo-200') : (isDark ? 'bg-zinc-700/50 text-zinc-300 border-zinc-600/50 hover:border-zinc-500' : 'bg-slate-100 text-slate-600 border-slate-200 hover:border-slate-300')}`}>CRC16</button>
                        <button onClick={() => setUseHexSend(!useHexSend)} className={`px-2 py-0.5 rounded border text-[8px] font-black transition-all ${useHexSend ? (isDark ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40 shadow-[0_0_10px_rgba(16,185,129,0.1)]' : 'bg-indigo-50 text-indigo-600 border-indigo-200') : (isDark ? 'bg-zinc-700/50 text-zinc-300 border-zinc-600/50 hover:border-zinc-500' : 'bg-slate-100 text-slate-600 border-slate-200 hover:border-slate-300')}`}>HEX</button>
                    </div>
                    <div className={`flex ${isDark ? 'bg-zinc-700/30 border-zinc-600/50' : 'bg-slate-100 border-slate-200'} border rounded overflow-hidden`}>
                        {[ {label:'\\n', val:'\\n'}, {label:'\\r\\n', val:'\\r\\n'} ].map(opt => (
                            <button key={opt.val} onClick={() => setLineEnding(opt.val)} className={`px-2 py-0.5 text-[8px] font-black transition-all ${lineEnding === opt.val ? (isDark ? 'bg-emerald-500/20 text-emerald-400' : 'bg-indigo-50 text-indigo-600') : (isDark ? 'text-zinc-300 hover:text-white' : 'text-slate-500 hover:text-slate-700')}`}>{opt.label}</button>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 px-1">
                    ...
                    <input
                      ...
                      className={`w-16 ${isDark ? 'bg-zinc-700/50 border-zinc-600/50 text-zinc-200 focus:border-emerald-500/40' : 'bg-white border-slate-200 text-slate-700 focus:border-indigo-500'} border rounded px-2 py-1 text-[10px] font-mono text-center outline-none`}
                    />
                ...
                <div className="relative group">
                    <textarea
                        ...
                        className={`w-full h-28 ${isDark ? 'bg-zinc-700/40 border-zinc-600/50 text-emerald-100 focus:border-emerald-500/40 placeholder:text-zinc-500' : 'bg-white border-slate-200 text-slate-800 focus:border-indigo-500 placeholder:text-slate-400'} border rounded-xl p-3 text-[12px] font-mono focus:outline-none transition-all resize-none placeholder:text-[10px]`}
                    />
                </div>
                <button
                  onClick={() => sendData()}
                  ...
                  className={`w-full py-3.5 bg-gradient-to-r ${t.btnPrimary} ${t.btnPrimaryHover} ${isDark ? 'disabled:from-zinc-700 disabled:to-zinc-700 disabled:text-zinc-500' : 'disabled:from-slate-200 disabled:to-slate-200 disabled:text-slate-500'} disabled:cursor-not-allowed text-white text-[10px] font-black tracking-[0.3em] rounded-xl transition-all shadow-lg ${isDark ? 'shadow-emerald-900/20' : 'shadow-indigo-500/20'} disabled:shadow-none flex items-center justify-center gap-2.5 active:scale-95 group`}
                >
        ...
        {/* Feedback Toast */}
        {copyFeedback && (
            <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 z-[100] ${isDark ? 'bg-zinc-800 border-emerald-500/40 text-emerald-400' : 'bg-white border-indigo-200 text-indigo-600'} border px-6 py-2 rounded-full text-[11px] font-black tracking-[0.3em] shadow-[0_15px_40px_rgba(0,0,0,0.3)] animate-in fade-in slide-in-from-bottom-4`}>
                {copyFeedback}
            </div>
        )}
      </div>
      ...
      {isConnectModalOpen && (
        <div className={`absolute inset-0 z-[200] flex items-center justify-center ${isDark ? 'bg-black/80' : 'bg-slate-900/50'} backdrop-blur-xl animate-in fade-in duration-300`}>
            <div className={`w-[540px] ${isDark ? 'bg-[#0c0c0e] border-white/20 ring-white/10' : 'bg-white border-slate-100 ring-indigo-100/60'} border rounded-[2.5rem] shadow-2xl overflow-hidden ring-1`}>
                <div className={`p-8 border-b ${isDark ? 'border-white/10 bg-zinc-800/50' : 'border-slate-100 bg-[#F8FAFC]'} flex items-center justify-between`}>
                    <div className="flex items-center gap-5">
                        <div className={`w-12 h-12 rounded-2xl ${isDark ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-indigo-50 border-indigo-100'} flex items-center justify-center border shadow-inner`}>
                            <Usb className={isDark ? 'text-emerald-400' : 'text-indigo-600'} size={24} />
                        </div>
                        <div>
                ...
                <div className={`p-8 ${isDark ? 'bg-black/20' : 'bg-white'}`}>
                    <div className="relative group mb-8">
                        ...
                        <input type="text" placeholder="Filter authorized devices..." value={portSearchQuery} onChange={e => setPortSearchQuery(e.target.value)} className={`w-full ${isDark ? 'bg-black/40 border-white/10 focus:border-emerald-500/40' : 'bg-white border-slate-200 focus:border-indigo-500 text-slate-700'} border rounded-2xl pl-12 pr-4 py-4 text-xs outline-none transition-all shadow-inner tracking-wider`} />
                    </div>
                    ...
                    <div className="max-h-80 overflow-y-auto custom-scrollbar flex flex-col gap-4 pr-2">
                        {availablePorts.length === 0 ? (
                            ...
                            availablePorts.map((p, i) => (
                                <button key={i} onClick={() => openPort(p)} className={`w-full text-left p-5 ${isDark ? 'bg-zinc-800/50 border-white/5 hover:border-emerald-500/50 hover:bg-zinc-700/60' : 'bg-white border-slate-200 hover:border-indigo-300 hover:bg-slate-50'} border rounded-3xl group transition-all flex items-center justify-between shadow-lg`}>
                                    <div className="flex items-center gap-6">
                                        <div className={`w-11 h-11 rounded-2xl ${isDark ? 'bg-black/60 border-white/5 group-hover:text-emerald-400 group-hover:bg-emerald-500/10' : 'bg-slate-50 border-slate-200 group-hover:text-indigo-600 group-hover:bg-indigo-50'} flex items-center justify-center transition-all border`}>
                                            <Usb size={20} />
                                        </div>
                                        <div>
                                            <div className={`text-[11px] font-black ${isDark ? 'group-hover:text-emerald-400' : 'text-slate-700 group-hover:text-indigo-600'} transition-colors uppercase tracking-tighter leading-none`}>Endpoint#{i+1}</div>
                                    ...
                                    <ChevronRight size={20} className={`${isDark ? 'text-zinc-800 group-hover:text-emerald-400' : 'text-slate-300 group-hover:text-indigo-500'} transition-all group-hover:translate-x-2`} />
                                </button>
                            ))
                        )}
                    </div>
                ...
                <div className={`p-8 ${isDark ? 'bg-[#0a0a0c] border-white/10' : 'bg-slate-50 border-slate-100'} border-t`}>
                    <button
                      ...
                      className={`w-full py-5 bg-gradient-to-r ${t.btnPrimary} ${t.btnPrimaryHover} text-white rounded-[1.5rem] text-[11px] font-black tracking-[0.4em] transition-all shadow-2xl ${isDark ? 'shadow-emerald-900/40' : 'shadow-indigo-500/20'} flex items-center justify-center gap-4 hover:scale-[1.01] active:scale-98`}
                    >
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: ${isDark ? 'rgba(16, 185, 129, 0.08)' : 'rgba(99, 102, 241, 0.15)'}; border-radius: 10px; border: 1px solid transparent; background-clip: padding-box; }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: ${isDark ? 'rgba(16, 185, 129, 0.25)' : 'rgba(99, 102, 241, 0.35)'}; }
      `}</style>
    </div>
  );
}
